...
AdcChSubsys::AdcChSubsys(EventQueue& eq) :
		m_eventQueue(eq) {
...
bool AdcChSubsys::addPin(Gpio pin) {
	...
	if (m_pins[static_cast<uint32_t>(pin)])
		return false;
...
void AdcChSubsys::runThread() {
	while (true) {
		if (m_subsysActive) {
			...
			adcConvert(&ADCD1, &m_adcConversionGroup, m_samples,
					kSampleBuffDepth);

			// package and post event
			std::vector < Event > events;
			events.push_back(
					Event(Event::Type::kAdcConversion, Gpio::kA1,
							0xfff & m_samples[0]));
			events.push_back(
					Event(Event::Type::kAdcConversion, Gpio::kA2,
							0xfff & m_samples[1]));
			events.push_back(
					Event(Event::Type::kAdcConversion, Gpio::kA3,
							0xfff & m_samples[2]));
			events.push_back(
					Event(Event::Type::kAdcConversion, Gpio::kA6,
							0xfff & m_samples[3]));
