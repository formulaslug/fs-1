...
void Vehicle::HandleADCs() {
  ...
  brakeVal = 100 - (100 * (brakeVoltage - kBrakeMin) / (kBrakeMax - kBrakeMin));
  ...
  steeringAngle =
      (100 * (steeringIn - kSteeringMin) / (kSteeringMax - kSteeringMin));
  //	arctan(total displacement (cm) /7 (cm)) = steering angle
}

uint8_t
Vehicle::FaultCheck() {  // checks if any fault states have tripped and puts
...
void Vehicle::FSM() {
  ...
  switch (state) {
    ...
    case kProfileSelect:
      ...
      } else if ((dashInputs & toggleUp) && (brakeVoltage > kBrakeThreshold) &&
                 (throttleVal <
                  kThrottleThreshold)) {  // Move to Drive Mode Delay if toggle
    ...
    case kDelayF:                  // Delay to shift into forward
      if ((dashInputs & toggleUp)  // Hold toggle up, foot off gas, foot on
          ...
          (throttleVal < kThrottleThreshold) && (timerDoneFlag & VT_SM_D)) {
        timerDoneFlag = 0;
        printf3("kFORWARD\n");
        state = kForward;
