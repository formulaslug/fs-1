...
class LTC6811 {
 public:
  enum class AdcMode : uint8_t { k27k, k14k, k7k, k3k, k2k, k1k, k422, k26 };
  ...
  static Command buildCommand(AddressingMode addrMode, uint8_t addr,
                              CommandCode cmd);
  static Command buildBroadcastCommand(CommandCode cmd);
  static Command toCommand(uint8_t val[2]);
  static Command toCommand(uint16_t val);

  LTC6811(SPIDriver *spiDriver, SPIConfig *spiConfig, uint8_t id);
